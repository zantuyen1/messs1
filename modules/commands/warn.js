module.exports.config = {
	...
	hasPermssion: 2,
	credits: "NTKhang",
	description: "Cảnh cáo thành viên  đủ 3 lần sẽ ban khỏi nhóm (nhớ set qtv cho bot nha), dùng warn help để xem cách dùng lệnh",
...
module.exports.run = async function({ api, args, Users, event, Threads, client }) {
  ...
  */
  ...
  if(args[0] == "help") {
    ...
    api.sendMessage(`Cách dùng: warn [key], với key là:\n${msg}`, threadID, messageID);
  }
  else if(args[0] == "view") {
  	if(!args[1]) {
  		var msg = "";
    ...
    if(args[1]) if(isNaN(args[1])) return api.sendMessage("❎id của người cần xóa khỏi danh sách bị cấm vào nhóm phải là con số", threadID, messageID);
	...
	if (!info.adminIDs.some(item => item.id == senderID) && !(global.config.ADMINBOT).includes(senderID)) return api.sendMessage('❎Chỉ qtv nhóm mới có thể sử dụng lệnh unban!', threadID, messageID);
  	if(!id) return api.sendMessage("❎Cần nhập id hoặc reply tin nhắn của người cần xóa khỏi danh sách bị cấm vào nhóm", threadID, messageID);
  	datawarn.banned;
	...
	if (!info.adminIDs.some(item => item.id == senderID) && !(global.config.ADMINBOT).includes(senderID)) return api.sendMessage('❎Chỉ qtv nhóm mới có thể sử dụng lệnh reset!', threadID, messageID);
  	...
  	api.sendMessage("Đã reset toàn bộ dữ liệu warn trong nhóm của bạn", threadID, messageID);
  	 ...
  	 //◆━━━━━━━━━◆WARN◆━━━━━━━━━◆\\
  	 	   ...
  	 	   if (event.type != "message_reply" && Object.keys(event.mentions).length == 0)	return global.utils.throwError(this.config.name, threadID, messageID);
