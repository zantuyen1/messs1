...
var min = Math.floor(Math.random() * 2);
  ...
  var money = data.money
   ...
   var callback = () => api.sendMessage({body:`Ảnh Cosplay Việt Nam\nSố Ảnh: ${link.length}\n-1000 đô !`,attachment: fs.createReadStream(__dirname + "/cache/49.jpg")}, event.threadID, () => fs.unlinkSync(__dirname + "/cache/49.jpg"), event.messageID); 
      return request(encodeURI(link[Math.floor(Math.random() * link.length)] + (max - min))).pipe(fs.createWriteStream(__dirname+"/cache/49.jpg")).on("close",() => callback());
     }
   };
