    ...
    const moneydb = (await Currencies.getData(victim)).money;
       var money = Math.floor(Math.random() * 1000) + 1;
        ...
        var moneyuser = (await Currencies.getData(event.senderID)).money
            if (moneyuser <= 0) return api.sendMessage("Vì Mặt Bạn Như Lồn Nên Không Thể Gạ Gẫm Được Ai Hãy Đi Phẫu Thuật Thẩm Mỹ Để Gạ Và Cắm Sừng Nhé", event.threadID, event.messageID);
