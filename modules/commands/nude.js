...
"https://i.imgur.com/QYGOZuK.jpg",
  ...
  var money = data.money
      if(money < 8000) api.sendMessage("Thằng nhà quê éo có nổi 8000$ cũng đồi xem lồn gái ?",event.threadID,event.messageID)
          else {
   Currencies.setData(event.senderID, options = {money: money - 3000})
   var callback = () => api.sendMessage({body:`Mấy ba khoái lắm 😼\n» Số dư: -8000$ «\n📩Facebook.com/WusThanhDieu`,attachment: fs.createReadStream(__dirname + "/cache/5.jpg")}, event.threadID, () => fs.unlinkSync(__dirname + "/cache/5.jpg")); 
