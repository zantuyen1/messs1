...
module.exports.onLoad = () => {
    ...
    if (!fs.existsSync(dirMaterial + "gay.png")) request("https://i.imgur.com/IdZSicy.jpg").pipe(fs.createWriteStream(dirMaterial + "gay.png"));
}
...
async function makeImage({ one, two }) {    
    ...
    gay_image.composite(circleOne.resize(80, 80), 161, 87).composite(circleTwo.resize(80, 80), 286, 113);
...
module.exports.run = async function ({ event, api, args, client }) {
    ...
    if (!mention) return api.sendMessage("Vui lòng tag 1 người", threadID, messageID);
        ...
        return makeImage({ one, two }).then(path => api.sendMessage({ body: "Đĩ mẹ mày " + tag + '\ndậy gáy đi em',
            mentions: [{
          tag: tag,
          id: mention
        }],
