...
module.exports.run = async function({ api, event, args }) {
	...
	let pathImg = __dirname + '/cache/canvas/nhat.png';
	var text = args.join(" ");
	if (!text) return api.sendMessage("Nhập nội dung cần tạo", threadID, messageID);
	let getTrum = (await axios.get(`https://imgur.com/a/zponHha.jpg`, { responseType: 'arraybuffer' })).data;
	fs.writeFileSync(pathImg, Buffer.from(getTrum, 'utf-8'));
	let baseImage = await loadImage(pathImg);
	let canvas = createCanvas(baseImage.width, baseImage.height);
	let ctx = canvas.getContext("2d");
	ctx.drawImage(baseImage, 0, 0, canvas.width, canvas.height);
	ctx.font = "400 25px Segoe UI";
