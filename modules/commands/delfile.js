module.exports.config = {
	...
	hasPermssion: 2,
	credits: "NTKhang",
	description: "Xóa file hoặc folder trong thư mục delfile",
...
•Ví dụ: delfile a
•Key: để trống
•Tác dụng: lọc ra tất cả các file trong delfile
•Ví dụ: delfile
•Key: help
...
•Ví dụ: delfile help`;
    ...
    if(files.length == 0) return api.sendMessage(`Không có file nào trong delfile có ký tự bắt đầu bằng: ${word}`, event.threadID ,event. messageID);
  ...
  //đuôi file là..... 
  	...
  	if(files.length == 0) return api.sendMessage(`Không có file nào trong delfile có ký tự kết thúc bằng: ${ext}`, event.threadID ,event. messageID);
  	var key = `Có ${files.length} file có đuôi là: ${ext}`;
  }
  //all file
  else if (!args[0]) {
  if(files.length == 0) return api.sendMessage("delfile của bạn không có file hoặc folder nào", event.threadID ,event. messageID);
  var key = "Tất cả các file trong thư mục delfile:";
  }
  //trong tên có ký tự.....
  else {
