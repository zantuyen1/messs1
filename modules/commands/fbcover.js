module.exports.config = {
	name: "taoanhbia", // Tên lệnh, được sử dụng trong việc gọi lệnh
...
module.exports.handleReply = async ({ api, event, handleReply }) => {
	...
	switch (handleReply.step) {
		case 1:
			content.name = input;
			sendC("Reply tin nhắn này nhập nickname của bạn", 2, content);
			break;
		case 2:
			content.subname = input;
			sendC("Reply tin nhắn này nhập số điện thoại của bạn", 3, content);
			break;
		case 3:
			content.number = input;
			sendC("Reply tin nhắn này nhập email của bạn", 4, content);
			break;
		case 4:
			content.email = input;
		...
		case 6:
			...
			const fs = require("fs");
			send("Thành công, đợi trong giây lát!");
			global.client.handleReply.splice(global.client.handleReply.indexOf(handleReply), 1);
			api.unsendMessage(handleReply.messageID);
			let c = content;
...
module.exports.run = ({ api, event, args }) => {
	...
	return api.sendMessage("Reply tin nhắn này rồi nhập họ tên của bạn", event.threadID, (err,info) => {
		global.client.handleReply.push({
			step: 1,
			name: this.config.name,
			messageID: info.messageID,
