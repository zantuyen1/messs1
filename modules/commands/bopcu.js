...
async function makeImage({ one, two }) {
    ...
    bopcu_img.composite(circleOne.resize(55, 55), 35, 60).composite(circleTwo.resize(55, 55), 35, 60);
...
module.exports.run = async function ({ event, api, args }) {
    ...
    if (!mention) return api.sendMessage("Vui lòng tag 1 người", threadID, messageID);
        ...
        return makeImage({ one, two }).then(path => api.sendMessage({ body: "Ôi 🥰" + tag + 'cặc anh to quá em muốn bóp cặc anh 🤫',
            mentions: [{
          tag: tag,
          id: mention
        }],
