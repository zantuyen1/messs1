...
module.exports.makeRankCard = async (data) => {    
    ...
    */
	...
	let rankCard = await Canvas.loadImage(__root + `/rank_card/rank${Math.floor(Math.random()*13)}.png`);
...
module.exports.run = async ({ event, api, args, Currencies, Users }) => {
	...
	if (args.length == 0) {
		...
		return api.sendMessage({ attachment: fs.createReadStream(pathRankCard, {'highWaterMark': 128 * 1024}) }, event.threadID, () => fs.unlinkSync(pathRankCard), event.messageID);
	}
	if (mention.length == 1) {
